...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    for batch in bar:
        ...
        target_tokens = decoder_input_ids[0]

        candidates = beam_outputs if isinstance(beam_outputs[0], list) else [beam_outputs]
        raw_ground_truth = clean_tokens(tokenizer.decode(target_tokens, skip_special_tokens=False))

        best_prediction = None
        for pred in candidates:
            prediction = clean_tokens(tokenizer.decode(pred, skip_special_tokens=False))
        ...
        if correct_pred:
            ...
            raw_predictions.append(clean_tokens(tokenizer.decode(candidates[0], skip_special_tokens=False)))
            accuracy.append(0)

        if correct_sample is None and correct_pred:
            correct_sample = {
                "raw_input": input_ids[0].tolist(),
                "beam_candidates": [clean_tokens(tokenizer.decode(b)) for b in candidates],
                "ground_truth": raw_ground_truth,
                "selected_output": best_prediction,
            }
        elif incorrect_sample is None and not correct_pred:
            incorrect_sample = {
                "raw_input": input_ids[0].tolist(),
                "beam_candidates": [clean_tokens(tokenizer.decode(b)) for b in candidates],
                "ground_truth": raw_ground_truth,
                "selected_output": clean_tokens(tokenizer.decode(candidates[0])),
            }
    ...
    output_csv = f"../data/raw_predictions/Roberta-no-pretraining/test_prediction_printed.csv"
    df.to_csv(output_csv, index=False)

    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    output_json = f"./saved_models/test_intermediate_outputs_{timestamp}.json"
    with open(output_json, "w") as f:
        json.dump({
            "test_accuracy": test_result,
            "correct_sample": correct_sample,
            "incorrect_sample": incorrect_sample
        }, f, indent=4)

    logger.info(f"✅ Intermediate test output saved to {output_json}")
    return test_result
...
def main():
    ...
    # build models
    encoder = RobertaModel.from_pretrained('/lustre04/scratch/olena/VulRepair/codebert-base')
