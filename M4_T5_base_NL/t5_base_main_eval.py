...
def evaluate(args, model, tokenizer, eval_dataset, eval_when_training=False):
    ...
    for batch in bar:
        ...
        if args.n_gpu > 1:
            loss = loss.mean()
        # Count the number of valid tokens (T5 uses -100 for padding/ignored positions in labels)
        non_pad_tokens = (labels != -100).sum().item()
        eval_loss += loss.item() * non_pad_tokens
        total_tokens += non_pad_tokens

    eval_loss = round(eval_loss / total_tokens, 5)
    #### CHANGES APPLIED HERE ####
